...
## Workflow IR-LG-Remote (Raspberry Pi)

Estas reglas aplican a este proyecto (emulador IR por infrarrojos).

### Principios

- La edición se hace localmente en `$PWD`; en la Pi solo `git pull`, build y
  ejecución.
- La compilación y las pruebas se ejecutan de forma **remota** en la Raspberry
  Pi (señala refleja también `docs/LEARNINGS.md`).

### Compilar y ejecutar en la Pi por SSH sin contraseña

Flujo de un ciclo completo con el repositorio en `/home/pi/src/ir_remote_rpi`:

```bash
# Clave pública (una sola vez)
ssh-copy-id pi@raspi.local

# Actualizar, compilar y ejecutar (recomendado, se invoca con `make remote`)
ssh pi@raspi.local "cd /home/pi/src/ir_remote_rpi && git pull && make clean && make -j4 && make run"
```

- Con argumentos: `make remote ARGS="--send POWER"` o
  `ssh pi@raspi.local "cd /home/pi/src/ir_remote_rpi && git pull && make clean && make -j4 && sudo ./bin/App --send POWER"`.
- Solo construir sin ejecutar: omítase el `make run` del comando.
- Sin clave pública puede usarse `sshpass` con `$SSHPASS`, pero preferir claves.

### Ciclo normal de una tarea

1. Editar código/documentación en `$PWD`.
2. Commit semántico local (`feat:`, `fix:`, `docs:`, ...).
3. Push a `origin/main`.
4. En la Pi: `git pull`, `make clean && make -j4` y probar sobre el TV.
5. Bump de `VERSION` y tag respetando las reglas de `LEARNINGS.md`
   (ciclo patch 0-9, `tag vX.Y.Z` = `VERSION X.Y.Z`).

### Notas propias del IR

- `make run` usa sudo (bcm2835 abre `/dev/mem`).
- Si el repositorio remoto en la Pi difiere de `origin`, `git pull` falla;
  usar `git pull --ff-only` para detectarlo pronto.
- Comprobar que no quede un proceso `App` vivo antes de reprogramar el pin
  (`pgrep App`).
