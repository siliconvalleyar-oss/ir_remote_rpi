...
## IR-LG-Remote — Aprendizajes específicos del proyecto

Secciones derivadas de la implementación de este emulador IR (VERSION 1.0.0).

### Compilación bcm2835 en la Raspberry Pi (32 y 64 bits)

- bcm2835 v1.71 se compila igual en `armhf` y `arm64` desde la fuente oficial.
  Después de `make install` queda en `/usr/local/include/bcm2835.h` y
  `/usr/local/lib/libbcm2835.a` (el Makefile lo busca en esa ruta por defecto).
- En un PC x86, para compilar en cruz, hay que traer esos dos archivos de la Pi
  a `sysroot/<triple>/` (script `scripts/fetch_sysroot.sh`);
  NO vale descargar la tarball y compilarla en x86 (no tiene código nativo útil).

### Acceso I2C en Raspberry Pi 5 / CM5

- La librería bcm2835 no da servicio I2C en equipos con chip RP1 (Pi 5/CM5).
  Por eso la OLED SSD1306 usa `ioctl` sobre `/dev/i2c-N` (capa `SSD1306_I2C`),
  independiente de bcm2835. El GPIO IR sí funciona con bcm2835 en la Pi 5.

### Temporización de la portadora 38 kHz

- `bcm2835_delayMicroseconds(13)` con dos llamadas por ciclo da ~26 µs
  (~38 kHz). La sobrecarga de `nanosleep` es simétrica en ambas mitades, por lo
  que el ciclo total se mantiene estable y dentro del ±20 % del receptor.
- Los tiempos NEC tienen margen; no es necesario compensar a nivel de µs.

### El receptor NEC (learn) valida por paridad

- Se descartan tramas con `addr xor ~addr != 0xFF` o `cmd xor ~cmd != 0xFF`.
  Esto evita códigos corruptos por ruido o distancia excesiva.

### Códigos LG

- La dirección NEC de LG suele ser `0x04`, pero algunos modelos usan `0x08`.
  Si un TV no responde, probar cambiando `NEC_ADDRESS` en
  `config/hardware.cfg` antes de tocar el código del comando.
- La tabla `config/ir_codes.cfg` es editable sin recompilar; verificar con
  `./bin/App --list`.
